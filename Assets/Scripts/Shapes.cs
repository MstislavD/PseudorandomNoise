...
{
    public delegate JobHandle SceduleDelegate(NativeArray<float3x4> positions, NativeArray<float3x4> normals,
    ...
    public struct Sphere : IShape
    {
        public Point4 GetPoint4(int i, float resolution, float invResolution)
        {
            ...
            float4 offset = max(0f, -p.positions.c2);
            p.positions.c0 += select(offset, -offset, p.positions.c0 > 0);
            p.positions.c1 += select(offset, -offset, p.positions.c1 > 0);
    ...
    static float4x2 IndexTo4UV(int i, float resolution, float invResolution)
    {
        float4x2 uv;
